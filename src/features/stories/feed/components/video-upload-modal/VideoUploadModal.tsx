    ...
    <Modal onClose={onClose} width={480} fitContent>
      <div className="flex flex-col bg-white w-full md:max-h-[90vh] h-full md:h-auto overflow-hidden">
        ...
        <div className="flex items-start justify-between p-6 flex-shrink-0 md:border-b-0">
          <div>
            <Text textLevel="h2" colorType='primary' size='text-md' weight='semibold'>{t('video.upload.title')}</Text>
            <Text textLevel="p" colorType='tertiary' size='text-sm'>{t('video.upload.subtitle')}</Text>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Content */}
        <div className="flex-1 px-6 overflow-y-auto flex flex-col gap-6 pb-6 md:pb-0">{/* Video Upload */}
          {/* Video Upload */}
        ...
        <div className="flex flex-row-reverse p-4 md:p-6 border-t border-neutral-200 flex-shrink-0" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
          <Button
            ...
            colorType="primary"
            size="lg"
            label={isUploading ? t('video.upload.publishing') : t('video.upload.publish')}
          />
        </div>
      </div>
    </Modal>
