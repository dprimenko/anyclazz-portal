...
import { Button, buttonVariants } from "@/ui-library/shared/button"
...
function Calendar({
  ...
  const [displayMonth, setDisplayMonth] = React.useState<Date>(props.month || today)
  
  const isCurrentMonth = disablePastMonths && 
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "group/calendar px-4 py-2.5 [--cell-size:--spacing(8)] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent",
        String.raw`rtl:**:[.rdp-button\_next>svg]:rotate-180`,
        String.raw`rtl:**:[.rdp-button\_previous>svg]:rotate-180`,
        className
      )}
      captionLayout={captionLayout}
      fromMonth={disablePastMonths ? currentMonth : undefined}
      formatters={{
      ...
      classNames={{
        ...
        day: cn(
          "relative w-full h-full p-0 text-center [&:first-child[data-selected=true]_button]:rounded-full [&:last-child[data-selected=true]_button]:rounded-r-md group/day aspect-square select-none",
      ...
      components={{
        ...
        Chevron: ({ className, orientation, ...props }) => {
          if (orientation === "left") {
            return (
              <ChevronLeftIcon 
                className={cn("size-4", isCurrentMonth && "invisible", className)} 
                {...props} 
              />
            )
          }
